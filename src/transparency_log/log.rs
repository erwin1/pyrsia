...
impl TransparencyLogService {
    ...
    pub async fn request_add_authorized_node(&mut self, peer_id: &str) -> Result<(), anyhow::Error> {
        let add_node_transparency_log = self
            .create_add_node(peer_id).unwrap();

        let payload = serde_json::to_string(&add_node_transparency_log).unwrap();

        let _ = self.blockchain_service
            .lock()
            .await
            .add_payload(payload.into_bytes())
            .await;
